...
std::string get_attr_rr(const std::string& svg, const std::string& attr) {
...
std::unique_ptr<GraphicsObject> RoundedRectangle::FromSvg(
  ...
  std::string val;
  if (!(val = get_attr_rr(svg, "x")).empty()) x = std::stod(val);
  if (!(val = get_attr_rr(svg, "y")).empty()) y = std::stod(val);
  if (!(val = get_attr_rr(svg, "width")).empty()) width = std::stod(val);
  if (!(val = get_attr_rr(svg, "height")).empty()) height = std::stod(val);
  if (!(val = get_attr_rr(svg, "rx")).empty()) radius = std::stod(val);
  if (!(val = get_attr_rr(svg, "stroke-width")).empty())
    stroke_width = std::stoi(val);
  if (!(val = get_attr_rr(svg, "fill")).empty()) fill_color = val;
  if (!(val = get_attr_rr(svg, "stroke")).empty()) stroke_color = val;
